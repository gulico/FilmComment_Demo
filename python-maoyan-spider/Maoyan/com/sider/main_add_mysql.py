...
def find_mysql():
    db= pymysql.connect(host="localhost",user="admin",password="123123",db="beanfilmdata",port=3306)
    cur = db.cursor()
...
def to_mysql(data):
    ...
    cursor = db.cursor()
    sql = 'INSERT INTO filmscore(`url`,`movie`,`ename`,`people`,`star`,`pf`) VALUES ('+data+')'
    print(sql)
    try:
        cursor.execute(sql)
        print("Successful")
